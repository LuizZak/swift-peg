extension GrammarProcessor {
    ...
    func computeUnreachableRules(
        ...
        guard let startNode = graph.nodes.first(where: { $0 == startRuleName }) else {
        ...
        let unreachableNodes = graph.nodes.filter({ unreachable.contains($0) })
